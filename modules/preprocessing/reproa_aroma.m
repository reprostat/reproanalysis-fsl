...
%
% Based on https://github.com/automaticanalysis/automaticanalysis/blob/master/aa_modules/aamod_AROMA_denoise.m
...
switch command
    ...
    case 'doit'
        ...
        working_dir = aas_getsesspath(rap, subj, run);

        save_fsloutputtype = rap.directoryconventions.fsloutputtype;
        rap.directoryconventions.fsloutputtype = 'NIFTI_GZ';
        % NB: AROMA seems to require full paths to everything
        [~, AROMA] = aas_cache_get(rap,'aroma');
        ...
        EPI_FNAME = aas_getimages_bystream(rap, subj, run, 'epi');
        STRUCTURAL_FNAME = aas_getfiles_bystream(rap, subj, 'structural');
        ...
        MC_FNAME = aas_getimages_bystream(rap, subj, run, 'realignment_parameter');
        ...
        REFERENCE_FNAME = fullfile(rap.directoryconventions.fsldir,'data/standard/MNI152_T1_2mm_brain.nii.gz');
        REFMASK_FNAME = fullfile(rap.directoryconventions.fsldir,'data/standard/MNI152_T1_2mm_brain_mask.nii.gz');
        ...
        aas_log(rap, false, sprintf('Brain extraction on %s...', STRUCTURAL_FNAME));
        command = sprintf('bet %s %s', STRUCTURAL_FNAME, BETTEDSTRUCTURAL_FNAME);
        [ status,result ] = aas_runfslcommand(rap, command);
        if (status > 0)
           aas_log(rap, true, sprintf('Error running bet: %s', result));
        end
        ...
        aas_log(rap, false, sprintf('Running FLIRT on %s...', BETTEDSTRUCTURAL_FNAME));
        command = sprintf('flirt -in %s -ref %s -omat %s', BETTEDSTRUCTURAL_FNAME, REFERENCE_FNAME, STRUCT2MNI_AFFINE);
        [ status,result ] = aas_runfslcommand(rap, command);
        if (status > 0)
           aas_log(rap, true, sprintf('Error running flirt: %s', result));
        end
        ...
        aas_log(rap, false, sprintf('Running FNIRT on %s...', STRUCTURAL_FNAME));
        command = sprintf('fnirt --in=%s --aff=%s --cout=%s --config=T1_2_MNI152_2mm', STRUCTURAL_FNAME, STRUCT2MNI_AFFINE, STRUCT2MNI_WARP);
        [ status,result ] = aas_runfslcommand(rap,command);
         if (status > 0)
            aas_log(rap, true, sprintf('Error running fnirt: %s', result));
         end
        ...
        aas_log(rap, false, sprintf('Running FLIRT on %s...', EPI_FNAME));
        ...
        aas_log(rap, false, sprintf('creating mean epi...'));
        command = sprintf('fslmaths %s -Tmean %s', EPI_FNAME, MEANEPI_FNAME);
        [ status,result ] = aas_runfslcommand(rap,command);
        if (status > 0)
           aas_log(rap, true, sprintf('Error creating mean epi: %s', result));
        end

        % DETREND - do a highpass filter based on 100s

        DICOMHEADERS = load(aas_getfiles_bystream(rap,subj, run,'epi_dicom_header'));
        ...
        WARPREF = fullfile(rap.directoryconventions.fsldir,'data/standard/MNI152_T1_2mm');
        aas_log(rap, false, sprintf('transforming %s to MNI space...', SMOOTHED_FNAME));
        ...
        [ status,result ] = aas_runfslcommand(rap,command);
        if (status > 0)
            aas_log(rap, true, sprintf('Error running applywarp: %s', result));
        end

        % now finally ready to run AROMA

        aas_log(rap, false, sprintf('Running AROMA on %s...', EPIMNI_FNAME));
        ...
        [ status,result ] = aas_runfslcommand(rap,command);
        if (status > 0)
            aas_log(rap, true, sprintf('Error running AROMA: %s', result));
        else
            aas_log(rap, false, result);
        end
        ...
        rap = aas_desc_outputs(rap, subj, run, 'epi', spm_file(output_fname,'path',working_dir));

        % cleanup

        if (rap.tasklist.currenttask.settings.deletegzips)
        ...
        rap.directoryconventions.fsloutputtype = save_fsloutputtype;
    ...
    case 'checkrequirements'
        ...
        % - conda environment
        [s,w] = runPyCommand('conda info -e','','quiet',true);
